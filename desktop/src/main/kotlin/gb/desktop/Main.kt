...
fun main(args: Array<String>) {
    ...
    if (args.size == 1 && args[0] == "--reset-video") {
...
/** Janela principal multi-sistema: biblioteca + emulação via [EmulatorCore]. */
class EmulatorWindow : JFrame() {
    ...
    private var prevFrame = IntArray(0)
    ...
    private val gamePanel = object : JPanel() {
        override fun paintComponent(g: Graphics) {
            ...
            val cw = core?.width ?: 160; val ch = core?.height ?: 144
            val f = minOf(width / cw.toDouble(), height / ch.toDouble()).coerceAtLeast(1.0)
            val w = (cw * f).toInt(); val h = (ch * f).toInt()
        ...
        override fun getPreferredSize() = Dimension((core?.width ?: 160) * scale, (core?.height ?: 144) * scale)
    }
    ...
    private fun showGame() { inGame = true; cards.show(root, "game"); if (!fullscreen) pack() }
    private fun showLibrary() { persistSave(); inGame = false; launcher.refresh(); cards.show(root, "launcher") }

    private fun gb(): GbCore? = core as? GbCore

    // ---------------- ROM ----------------
    private fun loadRom(file: File) {
        if (!file.exists()) { JOptionPane.showMessageDialog(this, "ROM não encontrada: ${file.path}"); return }
        val system = Systems.forFile(file)
        if (system == null) { JOptionPane.showMessageDialog(this, "Extensão não suportada: .${file.extension}"); return }
        persistSave()
        val bytes = file.readBytes()
        romBytes = IntArray(bytes.size) { bytes[it].toInt() and 0xFF }
        romFile = file
        newMachine(system.createCore)
        addRecent(file.absolutePath); rebuildRecentMenu()
        title = "Emulator — ${file.nameWithoutExtension} · ${system.name}"
    }
    ...
    private fun newMachine(factory: (IntArray, IntArray?) -> EmulatorCore) {
        ...
        val c = factory(bytes, save)
        core = c
        gb()?.gameBoy?.ppu?.dmgPalette = DmgPalettes.byName(paletteName)
        img = BufferedImage(c.width, c.height, BufferedImage.TYPE_INT_RGB)
        prevFrame = IntArray(c.width * c.height)
        if (autosave) romFile?.let { autoStateFile(it) }?.takeIf { it.exists() }?.let { c.loadState(it.readBytes()) }
        frameAccum = 0.0
    }

    private fun persistSave() {
        val c = core ?: return; val rf = romFile ?: return
        c.saveRam()?.let { snap -> saveFileFor(rf).writeBytes(ByteArray(snap.size) { snap[it].toByte() }) }
        if (autosave) autoStateFile(rf).writeBytes(c.saveState())
    }

    private fun saveStateSlot(slot: Int) {
        val c = core ?: return; val rf = romFile ?: return
        stateFileFor(rf, slot).writeBytes(c.saveState()); flash("Estado salvo no slot $slot")
    }
    private fun loadStateSlot(slot: Int) {
        val c = core ?: return; val rf = romFile ?: return
        val sf = stateFileFor(rf, slot)
        if (sf.exists()) { c.loadState(sf.readBytes()); flash("Slot $slot carregado") } else flash("Slot $slot vazio")
    }
    ...
    /** Correção de cor do LCD do CGB (leve dessaturação/escurecimento). Opcional, off por padrão. */
    ...
    private fun tick() {
        ...
        frameAccum += s
        while (frameAccum >= 1.0) { c.runFrame(); frameAccum -= 1.0 }

        val fb = c.framebuffer
        val cc = colorCorrect && gb()?.gameBoy?.cartridge?.isColor == true
        for (i in fb.indices) {
            val cur = if (cc) correct(fb[i]) else fb[i]
            val shown = if (ghosting) blend(cur, prevFrame[i]) else cur
            prevFrame[i] = shown
            img.setRGB(i % c.width, i / c.width, shown)
        }
        gamePanel.repaint()
    ...
    private fun buildMenu(): JMenuBar {
        ...
        file.addSeparator()
        file.add(JMenuItem("Reiniciar (Reset)").also { it.addActionListener { romFile?.let { f -> Systems.forFile(f)?.let { s -> newMachine(s.createCore) } } } })
        ...
        DmgPalettes.names.forEach { name ->
            ...
            item.addActionListener {
                paletteName = name; prefs.put("palette", name); gb()?.gameBoy?.ppu?.dmgPalette = DmgPalettes.byName(name)
            }
        ...
        val chMenu = JMenu("Canais (GB)")
        listOf("Onda 1", "Onda 2", "Wave", "Ruído").forEachIndexed { n, label ->
            chMenu.add(JCheckBoxMenuItem(label, true).also { it.addActionListener { m -> gb()?.gameBoy?.apu?.channelOn?.set(n, (m.source as JCheckBoxMenuItem).isSelected) } })
        }
    ...
    // ---------------- cheats (Game Boy) ----------------
    private fun cheatLabel(c: gb.Cheat) = (if (c.gameShark) "GameShark" else "GameGenie") + " @%04X=%02X".format(c.address, c.value)

    private fun cheatsDialog() {
        val m = gb()?.gameBoy?.memory
        if (m == null) { JOptionPane.showMessageDialog(this, "Cheats disponíveis apenas para Game Boy (por enquanto)."); return }
    ...
    // ---------------- input + remapeamento ----------------
    private fun defaultKeymap(): MutableMap<Int, Button> = mutableMapOf(
        KeyEvent.VK_RIGHT to Button.RIGHT, KeyEvent.VK_LEFT to Button.LEFT,
        KeyEvent.VK_UP to Button.UP, KeyEvent.VK_DOWN to Button.DOWN,
        KeyEvent.VK_Z to Button.A, KeyEvent.VK_X to Button.B,
        KeyEvent.VK_A to Button.X, KeyEvent.VK_S to Button.Y,
        KeyEvent.VK_Q to Button.L, KeyEvent.VK_W to Button.R,
        KeyEvent.VK_ENTER to Button.START, KeyEvent.VK_SHIFT to Button.SELECT,
    )

    private fun loadKeymap(): MutableMap<Int, Button> {
        ...
        KeyboardFocusManager.getCurrentKeyboardFocusManager().addKeyEventDispatcher { e ->
            ...
            when (e.id) {
                KeyEvent.KEY_PRESSED -> when (e.keyCode) {
                    ...
                    else -> keymap[e.keyCode]?.let { core?.setButton(it, true) }
                }
                KeyEvent.KEY_RELEASED -> when (e.keyCode) {
                    KeyEvent.VK_TAB -> turboHeld = false
                    else -> keymap[e.keyCode]?.let { core?.setButton(it, false) }
                }
            }
